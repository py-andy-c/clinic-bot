...
const SettingsRemindersPage: React.FC = () => {
      ...
      if (pendingStr === currentStr) {
        ...
        const performSave = async () => {
          try {
            ...
            isSavingRef.current = false;
            pendingFormDataRef.current = null;
            handleBackendError(err, methods as unknown as Parameters<typeof handleBackendError<RemindersSettingsFormData>>[1]);
          }
        };
        performSave();
      }
  ...
  const onFormSubmit = async (data: RemindersSettingsFormData) => {
    ...
    try {
      ...
      isSavingRef.current = false;
      pendingFormDataRef.current = null;
      if (!handleBackendError(err, methods as unknown as Parameters<typeof handleBackendError<RemindersSettingsFormData>>[1])) {
