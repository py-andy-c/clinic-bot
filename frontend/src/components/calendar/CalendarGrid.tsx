...
const CalendarEventComponent: React.FC<CalendarEventComponentProps> = ({
  ...
  const eventStyle = useMemo(() => {
    ...
    if (event.resource.practitioner_id) {
      backgroundColor = getPractitionerColor(event.resource.practitioner_id, currentUserId ?? -1, selectedPractitioners) || '#3b82f6';
    ...
    if (event.resource.type === 'availability_exception') {
      ...
      const practitionerColor = event.resource.practitioner_id
        ? getPractitionerColor(event.resource.practitioner_id, currentUserId ?? -1, selectedPractitioners) || '#3b82f6'
        : '#3b82f6';
...
const MonthlyBody: React.FC<MonthlyBodyProps> = ({
  currentDate,
  events,
  selectedPractitioners,
  selectedResources,
  ...
  return (
    <div className={styles.monthlyGrid} style={{ padding: 0 }}>
      {/* Header removed as it is handled by PractitionerRow */}
...
export const PractitionerRow: React.FC<Omit<CalendarGridProps, 'showHeaderRow'>> = (props) => {
  ...
  return (
    <div className={styles.headerRow} data-testid="calendar-header-row">
      {view !== CalendarViews.MONTH && <div className={styles.timeCorner} data-testid="calendar-time-corner"></div>}
      <div
