...
describe('ClinicalWorkspace', () => {
  ...
  describe('Tablet Interactions', () => {
    ...
    beforeEach(() => {
      ...
      stageElement = container.querySelector('.konvajs-content') as HTMLElement;
      if (!stageElement) throw new Error('Stage not found');
    });

    it('cancels drawing when a second finger touches the screen', async () => {
      fireEvent.click(screen.getByLabelText(/畫筆/i));

      // Start drawing with one finger
      const touch1 = { identifier: 1, clientX: 100, clientY: 100 };
      fireEvent.touchStart(stageElement, {
        touches: [touch1],
        changedTouches: [touch1],
      });

      // Move a bit
      const touch1Move = { identifier: 1, clientX: 110, clientY: 110 };
      fireEvent.touchMove(stageElement, {
        touches: [touch1Move],
      ...
      fireEvent.touchStart(stageElement, {
        touches: [touch1Move, touch2],
        changedTouches: [touch2],
      });

      // Drawing should be cancelled, so no new path should be added on touchEnd
      fireEvent.touchEnd(stageElement, {
        touches: [],
        changedTouches: [touch1Move, touch2],
      });

      expect(mockOnUpdate).not.toHaveBeenCalledWith(expect.objectContaining({
        layers: expect.arrayContaining([
          expect.objectContaining({ type: 'drawing' })
        ])
      }));
    });

    it('handles multi-touch panning', async () => {
      ...
      const touch1 = { identifier: 1, clientX: 100, clientY: 100 };
      const touch2 = { identifier: 2, clientX: 200, clientY: 100 };
      fireEvent.touchStart(stageElement, {
        touches: [touch1, touch2],
        changedTouches: [touch1, touch2],
      });

      // Move both fingers down by 50px
      const touch1Move = { identifier: 1, clientX: 100, clientY: 150 };
      const touch2Move = { identifier: 2, clientX: 200, clientY: 150 };
      fireEvent.touchMove(stageElement, {
        touches: [touch1Move, touch2Move],
        changedTouches: [touch1Move, touch2Move],
      });

      // Panning should trigger window.scrollTo
      // dx = 0, dy = 50. scrollTo(initialX - 0, initialY - 50)
      expect(scrollToSpy).toHaveBeenCalledWith(initialScrollX, initialScrollY - 50);
    });

    it('rejects palm-like touches (large radius)', async () => {
      fireEvent.click(screen.getByLabelText(/畫筆/i));

      // Touch with large radius (> 20px)
      const palmTouch = {
        identifier: 1,
        clientX: 100,
        clientY: 100,
      ...
      fireEvent.touchStart(stageElement, {
        touches: [palmTouch],
        changedTouches: [palmTouch],
      });

      fireEvent.touchMove(stageElement, {
        touches: [{ ...palmTouch, clientX: 110, clientY: 110 }],
        changedTouches: [{ ...palmTouch, clientX: 110, clientY: 110 }],
      });
    ...
    it('stays in panning mode until all fingers are lifted', async () => {
      fireEvent.click(screen.getByLabelText(/畫筆/i));
      ...
      // Lift one finger (touch2)
      fireEvent.touchEnd(stageElement, {
        touches: [touch1],
        changedTouches: [touch2],
      });

      // Move remaining finger (touch1)
      // Since we are still in panning mode (not drawing), this shouldn't start a drawing
      fireEvent.touchMove(stageElement, {
        touches: [{ ...touch1, clientX: 150, clientY: 150 }],
        changedTouches: [{ ...touch1, clientX: 150, clientY: 150 }],
      });
