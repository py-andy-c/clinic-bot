...
const PractitionerRow: React.FC<PractitionerRowProps> = ({
  ...
  const { register } = useFormContext<AppointmentsSettingsFormData>();
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
      <label className="flex items-center space-x-3 cursor-pointer flex-1">
        <input
          type="checkbox"
          {...register(`practitioners.${index}.patient_booking_allowed`)}
...
const ClinicAppointmentSettings: React.FC<ClinicAppointmentSettingsProps> = ({
  ...
  const {
    ...
    watch,
    setValue,
  } = useFormContext<AppointmentsSettingsFormData>();
