...
interface ServiceTypeGroupsTableProps {
  ...
  onCancelAdd?: () => void;
  // Legacy props that might be passed but not used (to prevent type errors if parent hasn't fully updated TS checks yet, though we updated parent)
...
export const ServiceTypeGroupsTable: React.FC<ServiceTypeGroupsTableProps> = ({
  ...
  const handleDragOver = (e: React.DragEvent, targetGroupId: number) => {
    ...
    if (!draggedGroupId || !onMove || draggedGroupId === targetGroupId) return;

    const dragIndex = groups.findIndex(g => g.id === draggedGroupId);
    const hoverIndex = groups.findIndex(g => g.id === targetGroupId);
    ...
    return (
      <React.Fragment key={group.id}>
        <tr
          ...
          className={`hover:bg-gray-50 transition-colors transition-all duration-200 ${draggedGroupId === group.id
              ...
              {isEditing ? (
                <div>
                  <input
                    ...
                    onKeyDown={(e) => handleKeyDown(e, group)}
                    className={`block w-full px-3 py-1.5 text-sm text-gray-900 bg-white border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all shadow-sm ${errorMessage ? 'border-red-500' : 'border-blue-300'
                      }`}
                    placeholder="群組名稱"
                  />
                  {errorMessage && (
        ...
        <div className="bg-blue-50/50 border-2 border-dashed border-blue-200 rounded-lg p-4 shadow-sm animate-pulse-slow">
          <div className="flex items-center gap-3">
            <input
              ...
              className={`block w-full px-3 py-2 text-sm text-gray-900 bg-white border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none ${errorMessage ? 'border-red-500' : 'border-blue-300'
                }`}
              placeholder="輸入新群組名稱"
            />
            {errorMessage && (
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <input
                ...
                onKeyDown={(e) => handleKeyDown(e, dummyGroup)}
                className={`block w-full px-3 py-1.5 text-sm text-gray-900 bg-white border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all shadow-sm max-w-xs ${errorMessage ? 'border-red-500' : 'border-blue-300'
                  }`}
                placeholder="輸入新群組名稱"
              />
              {errorMessage && (
