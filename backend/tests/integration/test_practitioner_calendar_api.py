...
class TestPractitionerCalendarAPI:
    ...
    def test_get_calendar_daily_view_with_missing_appointment_type(self, client: TestClient, db_session: Session):
        """Test getting calendar data for daily view when appointment has missing appointment_type."""
        # Create test clinic and practitioner
        clinic = Clinic(
            name="Test Clinic",
            line_channel_id="test_channel",
        ...
        practitioner = User(
            ...
            google_subject_id="practitioner_subject2",
            full_name="Dr. Test",
            roles=["practitioner"]
        )
        db_session.add(practitioner)
        ...
        # Create calendar event and appointment WITHOUT appointment_type
        # This simulates the case where appointment_type was deleted or is missing
        calendar_event = CalendarEvent(
            user_id=practitioner.id,
            event_type="appointment",
            date=date(2025, 1, 16),
        ...
        # Create appointment with a non-existent appointment_type_id (simulating deleted type)
        # We'll use a large ID that doesn't exist, then set the relationship to None
        appointment = Appointment(
            calendar_event_id=calendar_event.id,
            patient_id=patient.id,
        ...
        # Set the relationship to None to simulate missing appointment_type
        appointment.appointment_type = None
        db_session.commit()
        ...
        # Test getting daily calendar data - should not crash
        response = client.get(
            f"/api/clinic/practitioners/{practitioner.id}/availability/calendar?date=2025-01-16",
            headers={"Authorization": f"Bearer {token}"}
        )
        ...
        # Check appointment event - should handle missing appointment_type gracefully
        appointment_event = next(e for e in data["events"] if e["type"] == "appointment")
        assert appointment_event["start_time"] == "10:00"
        assert appointment_event["end_time"] == "11:00"
        # Title should show "未設定" (not set) for missing appointment_type
        assert appointment_event["title"] == "Test Patient - 未設定"
        assert appointment_event["appointment_type_name"] is None
        assert appointment_event["appointment_type_id"] == 99999

    def test_create_availability_exception_with_missing_appointment_type(self, client: TestClient, db_session: Session):
        """Test creating availability exception when conflicting appointment has missing appointment_type."""
        # Create test clinic and practitioner
        clinic = Clinic(
            name="Test Clinic",
            line_channel_id="test_channel",
        ...
        practitioner = User(
            ...
            google_subject_id="practitioner_subject3",
            full_name="Dr. Test",
            roles=["practitioner"]
        )
        db_session.add(practitioner)
        ...
        # Create appointment with missing appointment_type
        calendar_event = CalendarEvent(
            user_id=practitioner.id,
            event_type="appointment",
            date=date(2025, 1, 17),
        ...
        # Try to create availability exception that conflicts with appointment
        # Should show conflict warning with appointment_type as None
        ...
        data = response.json()
        assert "conflict" in data["detail"].lower() or "衝突" in data["detail"]
        
        # Verify that the conflict detection works even with missing appointment_type
        # The API should not crash when checking conflicts
