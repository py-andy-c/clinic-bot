                ...
                if config.timing_type == 'before':
                    if scheduled_time < current_time:
                        ...
                        if config.on_impossible == 'skip':
                            ...
                            # Send immediately (use current time + 1 minute to avoid race conditions)
                            scheduled_time = current_time + timedelta(minutes=1)
                            logger.info(
                                f"Adjusted patient form config {config.id} scheduled time to immediate send "
                                f"(original: {scheduled_time}, new: {scheduled_time})"
                            )
