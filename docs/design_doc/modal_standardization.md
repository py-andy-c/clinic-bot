...
- **Form modals with actions (create/edit forms)**
  ...
  - Hide the top-right "X" by default (`showCloseButton={false}`) to avoid redundancy and accidental dismissals.
...
5. Apply Close Policy:
   - Edit/create forms: `showCloseButton={false}`; provide 取消/儲存 in Footer.
...
## Targeted Components
- Already aligned: ServiceItemEditModal (fullscreen), ResourceTypeEditModal, SettingsAppointmentsPage info modal.
- Next to standardize:
  - MessagePreviewModal
  - InfoModal
  - PractitionerNotificationTimeSettings (info modal)
  - Calendar modals: NotificationModal, PractitionerSelectionModal, ServiceItemSelectionModal, CancellationNoteModal, CancellationPreviewModal, ConflictModal, ConflictWarningModal, EventModal, EditAppointmentModal, CheckoutModal, ReceiptListModal, ReceiptViewModal
  - ModalContext alert/confirm rendering
