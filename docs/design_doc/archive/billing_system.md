...
- `amount >= 0` (allows free services/zero amount checkout)
- `revenue_share >= 0`
...
- **Validation**: 
  ...
  - All items must have amount >= 0 (allows free services/zero amount checkout)
  - Payment method must be provided and valid
  - Billing scenarios must exist and not be deleted (if provided)
  - Service items must exist and not be soft-deleted
  - Quantity must be between 1 and 999 (enforced by Pydantic model)
...
- **Receipt Number Generation**: 
  ...
  - Uses current date's year in Taiwan timezone (not UTC, not appointment date) for receipt number
  - **Year Transition**: If checkout spans year boundary, receipt number uses year from `issue_date` in Taiwan timezone (ensures correct year even at year boundaries)
  - **Sequence Exhaustion**: System proactively checks if sequence is exhausted (>= 99,999) before generating receipt number and raises error if limit reached
  - **Edge Case**: If sequence reaches 99,999 in a year, system raises error and alerts admin (future enhancement: support 6+ digits or alternative numbering)
- **Snapshot Creation**:
...
- Service items must exist and not be soft-deleted
- Quantity must be between 1 and 999 (enforced by Pydantic model)
- Receipt number sequence must not be exhausted (proactive check before generation)
...
**Generation Logic:**
1. Get current year in Taiwan timezone (not UTC) to ensure correct year at year boundaries
2. Use PostgreSQL sequence or atomic counter for clinic + year
3. Check if sequence is exhausted (>= 99,999) before generating (proactive check)
4. Increment sequence number atomically
5. Format as `{YYYY}-{NNNNN}` with zero-padding (5 digits)
...
- **Mitigation**: 
  - Proactive check: System checks sequence value before generating receipt number
  - Raises `ValueError` if sequence is exhausted (>= 99,999), preventing invalid receipt numbers
  - Monitor receipt count and alert admin when approaching limit (e.g., >90,000) via accounting dashboard
...
- **Behavior**: Receipt number uses year from `issue_date` in Taiwan timezone (current timestamp when receipt is created)
- **Result**: Receipt number reflects actual issue date in Taiwan timezone, not appointment date
- **Rationale**: Receipts are issued at checkout time, not appointment time. Using Taiwan timezone ensures correct year even at year boundaries (e.g., UTC midnight vs Taiwan midnight)
- **Implementation**: Use Taiwan timezone when extracting year for receipt number generation (atomic operation)
- **Edge Case**: If checkout occurs at exactly midnight on New Year's Eve in Taiwan timezone, receipt number will use the new year (correct behavior)
...
### 4a. Soft-Deleted Service Items During Checkout
- **Scenario**: Service item is soft-deleted (is_deleted = true) but checkout is attempted
- **Behavior**: Checkout validation filters out soft-deleted service items, preventing checkout with deleted items
- **Validation**: System checks `is_deleted = False` when fetching service items during checkout
- **Error**: Returns `ValueError` with message "Service item {id} not found or has been deleted"
- **Rationale**: Maintains data integrity by preventing checkout with deleted service items
