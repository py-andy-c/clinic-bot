...
* \[x] Required field validation:
  * **Patient Forms (LIFF)**: Strict blocking validation with red error messages
  * **Clinic Forms**: Non-blocking with "warn on save" - post-save alert lists missing fields
* \[x] Appointment linking UI: Dropdown selector for patient appointments
* \[x] Validation feedback: Field-level errors (patient forms) and post-save warnings (clinic forms)
